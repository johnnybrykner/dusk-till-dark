...
account.$subscribe((mutation, state) => {
  console.log(state.userAccount?.account_settings.streaming_countries, state.userAccount?.account_settings.streaming_providers);
});
...
function handleCountryInput(country: StreamingCountry) {
...
function handleProviderInput(provider: StreamingProvider) {
  if (!account.userAccount) return;
